...
export default function PayrollDashboard() {
    ...
    // GETTING EMPLOYEES NAME AND ORIGINAL ID
    const employeeMap = useMemo(() => {
        const map = {};
        employees.forEach((employee) => {
            const info = {
                name: employee.name || employee.employeeName || (employee.email ? employee.email.split('@')[0] : "Employee"),
                department: employee.employeeRole || employee.role || employee.department || "General",
    ...
    const handleExportCSV = () => {
        ...
        const headers = ["Employee Name", "Employee ID", "Department", "Gross Pay", "Deductions", "Net Pay", "Status", "Date"];
        const rows = filteredPayslips.map((emp) => [
            `"${employeeMap[emp?.employeeId]?.name || emp?.employeeName || 'Employee'}"`,
            `"${employeeMap[emp?.employeeId]?.empId || emp?.employeeId || ''}"`,
            `"${employeeMap[emp?.employeeId]?.department || 'General'}"`,
            emp?.gross || 0,
            emp?.deductions || emp?.totalDeductions || 0,
            emp?.net || 0,
            `"${emp?.status || 'Pending'}"`,
    ...
    const statsSummary = useMemo(() => {
        if (!payslips || payslips.length === 0) {
            return { gross: 0, deductions: 0, net: 0, pendingCount: 0 };
        }
        return payslips.reduce(
            (acc, curr) => {
                acc.gross += Number(curr.gross || 0);
                acc.deductions += Number(curr.totalDeductions || curr.deductions || 0);
                acc.net += Number(curr.net || 0);
    ...
    const handleStatusChange = async (e, payslipId, currentStatus) => {
        e.stopPropagation();
                ...
                <div className="p-4 sm:p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <motion.div className="bg-white p-4 rounded-xl shadow border border-gray-100">
                            ...
                            <h2 className="text-xl font-bold text-blue-600">₹ {statsSummary.gross.toLocaleString('en-IN')}</h2>
                        </motion.div>
                        ...
                        <motion.div className="bg-white p-4 rounded-xl shadow border border-gray-100">
                            ...
                            <h2 className="text-xl font-bold text-red-500">₹ {statsSummary.deductions.toLocaleString('en-IN')}</h2>
                        </motion.div>
                        ...
                        <motion.div className="bg-white p-4 rounded-xl shadow border border-gray-100">
                            ...
                            <h2 className="text-xl font-bold text-green-600">₹ {statsSummary.net.toLocaleString('en-IN')}</h2>
                        </motion.div>
                        ...
                        <motion.div className="bg-white p-4 rounded-xl shadow border border-gray-100">
                            ...
                            <p className="text-gray-500 text-sm mt-2">Pending Payslips</p>
                            <h2 className="text-xl font-bold text-amber-600">{statsSummary.pendingCount}</h2>
                        </motion.div>
                ...
                <div className="p-4 sm:p-6 bg-[#f3f0eb] min-h-screen">
                    ...
                    <div className="flex flex-col lg:flex-row justify-between bg-white p-4 sm:p-5 rounded-2xl items-start lg:items-center gap-4 mb-6 shadow-sm border border-gray-100">
                        ...
                        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                            ...
                            {/* ADD BUTTON */}
                            <button className="flex items-center justify-center gap-2 bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 px-4 py-2.5 rounded-xl transition shadow-sm"
                            ...
                            <tbody>
                                {currentFiles?.map((emp, i) => (
                                    <tr key={i} className="border-t hover:bg-gray-50 cursor-pointer"
                                        ...
                                        <td className="p-4 font-medium">{employeeMap[emp?.employeeId]?.name || emp?.employeeName || "Employee"}</td>
                                        <td className="font-mono text-xs font-semibold text-slate-700">{employeeMap[emp?.employeeId]?.empId || (emp?.employeeId ? `EMP-${String(emp.employeeId).slice(-4).toUpperCase()}` : "EMP ID")}</td>
                                        <td>{employeeMap[emp?.employeeId]?.department || "General"}</td>
                                        <td className="text-blue-600 font-semibold">₹ {emp?.gross}</td>
                                        <td className="text-red-500 font-semibold">₹ {emp?.deductions || emp?.totalDeductions || 0}</td>
                                        <td className="text-green-600 font-semibold">₹ {emp?.net}</td>
                                        ...
                                        <td onClick={(e) => e.stopPropagation()}>
                                            <div className="flex items-center gap-2">
                                                <button
                                                    onClick={(e) => handleStatusChange(e, emp._id, emp.status)}

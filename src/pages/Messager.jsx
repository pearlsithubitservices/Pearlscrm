...
export default function Messenger() {
    ...
    const isUserMe = (idOrObj) => {
        ...
        const pId = typeof idOrObj === "object"
            ? (idOrObj._id || idOrObj.id || idOrObj.uid || idOrObj.email)
            : String(idOrObj);
        ...
        return Boolean(
            ...
            pId === user?.email ||
            pId === user?.username ||
            (pId === "admin" && (user?.role === "Admin" || user?.role === "admin" || userId === "admin"))
        );
    };
    ...
    const getChatDisplayName = (c) => {
        ...
        if (c.isGroup || (c.chatName && c.chatType !== "direct" && c.chatType !== undefined)) {
            return c.chatName || "Group Chat";
        }

        // Find participant that isn't current user
        const otherPart = c.participants?.find((p) => !isUserMe(p));

        if (typeof otherPart === "object" && (otherPart?.username || otherPart?.name || otherPart?.employeeName || otherPart?.email)) {
            return otherPart.username || otherPart.name || otherPart.employeeName || otherPart.email;
        }

        const otherId = typeof otherPart === "string" ? otherPart : (otherPart?._id || otherPart?.id || otherPart?.uid);
        if (otherId === "admin" && !isUserMe("admin")) return "Admin";
        const foundEmp = employees.find((emp) => (emp._id || emp.id || emp.uid || emp.email) === otherId);
        if (foundEmp) {
            return foundEmp.username || foundEmp.name || foundEmp.displayName || foundEmp.employeeName || (foundEmp.email ? foundEmp.email.split("@")[0] : "User");
        }
    ...
    const getParticipantDetails = (pIdOrObj) => {
        if (typeof pIdOrObj === "object" && (pIdOrObj.username || pIdOrObj.name || pIdOrObj.employeeName)) {
            return {
                name: pIdOrObj.username || pIdOrObj.name || pIdOrObj.employeeName,
                role: pIdOrObj.role || pIdOrObj.employeeRole || "User",
                email: pIdOrObj.email || "",
            };
        }
        const id = typeof pIdOrObj === "string" ? pIdOrObj : (pIdOrObj?._id || pIdOrObj?.id || pIdOrObj?.uid);
        if (isUserMe(id)) {
            return { name: user?.displayName || user?.username || user?.name || "You", role: user?.role || "Admin", email: user?.email || "" };
        }
        const emp = employees.find((e) => (e._id || e.id || e.uid || e.email) === id);
        if (emp) {
            return {
                name: emp.username || emp.name || emp.displayName || emp.employeeName || (emp.email ? emp.email.split("@")[0] : "User"),
                role: emp.role || "User",
                email: emp.email || "",
            };
        }
    ...
    const filteredChats = chats.filter((c) => {
        ...
        const isTaskChat = c.chatType === "task" || Boolean(c.taskId);
        const isGroupOrCollab = c.isGroup || c.chatType === "collab" || c.chatType === "group" || c.chatType === "channel";
        const isDirectChat = !isTaskChat && !isGroupOrCollab;

        // 1. Filter by Active Tab
        if (activeTab === "Chats") {
            // Show all chats in the main "Chats" tab
        } else if (activeTab === "Task Chats") {
            if (!isTaskChat) return false;
        } else if (activeTab === "Collabs") {
            if (!isGroupOrCollab) return false;
        }
    ...
    // Auto-select first chat in list only if no chat is active
    useEffect(() => {
        if (!activeChatId && filteredChats && filteredChats.length > 0) {
            setActiveChatId(filteredChats[0]._id);
        }
    }, [activeTab]);

    // FETCH USERS (for picking who to start a chat with from MongoDB Users DB)
    const fetchEmployees = async () => {
        try {
            ...
            try {
                ...
                console.log("MongoDB fetch users error:", err);
            }
    ...
    const handleCreateChat = async () => {
        ...
        const participants = newChatMode === "direct"
            ? Array.from(new Set([userId || "admin", selectedEmployeeIds[0]]))
            : Array.from(new Set([userId || "admin", ...selectedEmployeeIds]));
        ...
        if (newChat?._id) {
            if (chatType === "collab" || chatType === "group" || chatType === "channel") {
                            ...
                            {filteredChats.map((chat) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`group relative w-full flex items-start gap-3 px-4 py-3 text-left hover:bg-gray-50 cursor-pointer transition border-b border-gray-100 ${activeChatId === chat._id ? "bg-blue-50 border-l-4 border-l-blue-600" : ""
                                            }`}
                                    >
                                ...
                                <div className="flex items-center justify-between px-5 py-3 bg-white/70 backdrop-blur border-b border-gray-200">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            className={`p-2 rounded-lg transition ${showAbout ? "bg-blue-100 text-blue-600" : "hover:bg-white/80 text-gray-600"
                                                }`}
                                        >
                                ...
                                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 page-scroll">
                                    ...
                                    {messages.map((msg) => {
                                        const senderIdStr = typeof msg.senderId === "object"
                                            ? (msg.senderId?._id || msg.senderId?.id || msg.senderId?.uid || msg.senderId?.email)
                                            : String(msg.senderId || "");

                                        const isMine = Boolean(
                                            senderIdStr &&
                                            (senderIdStr === userId ||
                                             senderIdStr === user?._id ||
                                             senderIdStr === user?.id ||
                                             senderIdStr === user?.uid ||
                                             senderIdStr === user?.email)
                                        );
                                                                ...
                                                                {msg.attachments && msg.attachments.length > 0 && (
                                                                    <div className="mt-2 space-y-1.5">
                                                                        {msg.attachments.map((att, idx) => {
                                                                            const isImage = (att.type && att.type.startsWith("image/")) || (att.data && att.data.startsWith("data:image/"));
                                                                            const fileUrl = att.data || att.url;

                                                                            return (
                                                                                <div
                                                                                    key={idx}
                                                                                    className={`rounded-lg overflow-hidden border ${isMine
                                                                                    ...
                                                                                    {isImage && fileUrl ? (
                                                                                        <a href={fileUrl} target="_blank" rel="noopener noreferrer">
                                                                                            <img
                                                                                                ...
                                                                                                className="max-w-xs max-h-60 object-cover rounded-md hover:opacity-95 transition cursor-pointer"
                                                                                            />
                                                                                        </a>
                                                                                    ) : (
                                                                                        <div className="flex items-center gap-3 p-2.5 text-xs">
                                                                                            <div className={`p-2 rounded-lg ${isMine ? "bg-white/20 text-white" : "bg-blue-50 text-blue-600"}`}>
                                                                                                <FileText size={18} />
                                                                                            </div>
                                                                                            <div className="flex flex-col min-w-0 flex-1">
                                                                                                <span className="font-semibold truncate max-w-[150px]">{att.name || "File Attachment"}</span>
                                                                                                {att.size && (
                                                                                            ...
                                                                                            {fileUrl && (
                                                                                                <a
                                                                                                    ...
                                                                                                    className={`p-1.5 rounded-lg transition ${isMine ? "hover:bg-white/20 text-white" : "hover:bg-gray-100 text-blue-600"
                                                                                                        }`}
                                                                                                    title="Download File"
                                                                                                >
                                                                                                    <Download size={16} />
                                        ...
                                        Array.from(
                                            new Map(
                                                activeChat.participants.map((p) => {
                                                    ...
                                                    const key = details.name === "You" ? "CURRENT_USER_YOU" : (details.name || String(p));
                                                    return [key, details];
                                                })
                                            ).values()
                                        ).map((details, idx) => (
                            ...
                            {newChatMode === "task" && (
                                <>
                                    ...
                                    {tasks.map((t) => {
                                        ...
                                        const assigneeName = typeof t.assignedTo === "object"
                                            ? (t.assignedTo.name || t.assignedTo.employeeName || t.assignedTo.email || "Unassigned")
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`flex items-center gap-3 py-2.5 px-3 rounded-lg cursor-pointer transition mb-1 border ${isSelected
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col min-w-0 flex-1">
                                                    ...
                                                    <span className="text-xs text-gray-500 truncate">
                                                        Assigned: <strong className="text-gray-700">{assigneeName}</strong> | Status: {t.status || "Pending"}
                                                    </span>
                                                </div>
                                            </div>
                                        );
                                ...
                                <>
                                    ...
                                    {employees
                                        ...
                                        .map((emp) => {
                                            ...
                                            const empSub = emp.role || emp.email || "User";
                                        ...
                                        tasks.map((t, idx) => (
                                            <tr key={t._id || idx} className="hover:bg-gray-50/50 transition">
                                                ...
                                                <td className="py-3.5 px-6 text-center">
                                                    <span
                                                        className={`inline-block px-3 py-1 rounded-full text-[11px] font-bold ${t.status === "Completed"

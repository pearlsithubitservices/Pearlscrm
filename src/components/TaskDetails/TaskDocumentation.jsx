...
export default function TaskDocuments({ task, tasks }) {
  const { user } = useAuth();
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      formData.append(
        "uploadedBy",
        user?.displayName || user?.name || user?.employeeName || "Admin"
      );
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Upload files, specs, or contract documents for this task</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            <input
              ...
              id="admin-task-file-input"
              className="hidden"
            />
            <label
              htmlFor="admin-task-file-input"

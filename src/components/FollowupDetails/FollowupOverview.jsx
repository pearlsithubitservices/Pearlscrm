...
const FollowupOverview = ({ followups, fetchfollowups }) => {
  ...
  const employeeName = useMemo(() => {
    ...
    const found = employees.find(
      (emp) => emp.uid === followups.assignedTo || emp._id === followups.assignedTo || emp.name === followups.assignedTo
    );
  ...
  const handleMarkDone = async () => {
    ...
    try {
      await updateFollowup(followupId, {
        status: "Completed",
        isCompleted: true,
        newNote: "Marked as Completed from Overview",
        author: "Admin",
      });
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#0b2d59]">
                {followups?.type || "Call"} · {followups?.date || "Scheduled"} · {followups?.followupTime || "Pending"}
              </h1>
              <p className="text-sm text-[#8c8c8c] mt-1">
                Status: <span className={`font-semibold ${isCompleted ? "text-green-600" : "text-amber-600"}`}>

...
export default function LeadManagement() {
  ...
  const currentLead = (Array.isArray(leaddetails) ? leaddetails : []).filter(
    (item) => {
      ...
      const aToId =
        typeof aTo === "object" && aTo !== null
          ? aTo._id || aTo.id || aTo.uid || aTo.email
          : String(aTo || "");
      const aEmpId =
        typeof aEmp === "object" && aEmp !== null
          ? aEmp._id || aEmp.id || aEmp.uid || aEmp.email
          : String(aEmp || "");

      return (
        (userUid && (aToId === userUid || aEmpId === userUid)) ||
        (userEmail &&
          (aToId.toLowerCase() === userEmail.toLowerCase() ||
            aEmpId.toLowerCase() === userEmail.toLowerCase()))
      );
    },
  );
  ...
  const filteredLeads = useLeadfilter(
    currentLead,
    search,
    buttons[active],
    extraFilters,
  ...
  const activeExtraFilterCount = Object.values(extraFilters).filter(
    (v) => v !== "all",
  ).length;
  ...
  const notifications = useMemo(() => {
    ...
    currentLead.forEach((lead) => {
      ...
      const followUpStr =
        lead.nextActionDate || lead.follow || lead.followUpDate;

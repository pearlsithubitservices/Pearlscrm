...
export default function EmpFollowupNextaction({ followup, onRefresh }) {
    ...
    try {
      await updateFollowup(followupId, {
        ...
        newNote: noteText.trim()
          ? `Employee Action Plan (${date || "Date"} ${followupTime || ""}): ${noteText.trim()}`
          : `Rescheduled by employee for ${date || "upcoming date"} ${followupTime || ""}`,
        author: authorName,
      });

      toast.success("Employee next action saved successfully!");
      setNoteText("");
  ...
  return (
    ...
    >
      <h1 className="text-xs font-bold tracking-wide text-[#999] uppercase">
        NEXT ACTION & RESCHEDULE (EMPLOYEE PORTAL)
      </h1>
      ...
      <div className="mt-6">
        <h1 className="text-[#0b2d59] font-bold text-xs uppercase tracking-wider mb-2">
          Action Remarks / Customer Discussion
        </h1>
        <textarea
          value={noteText}
          onChange={(e) => setNoteText(e.target.value)}
          placeholder="Type your action plan, client response, or discussion summary..."
      ...
      <div className="mt-12 space-y-4">
        <h2 className="text-xs font-bold tracking-wide text-[#999] uppercase">
          ACTION LOGS & HISTORY ({historyLogs.length})
        </h2>

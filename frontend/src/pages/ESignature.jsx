...
export default function ESignature() {
  ...
  // Notification menu state & dismissed notifications persistent in localStorage (matching Tasks.jsx)
  const [showNotificationMenu, setShowNotificationMenu] = useState(false);
  const notifRef = useRef(null);
  const [dismissedNotifIds, setDismissedNotifIds] = useState(() => {
    try {
      const saved = localStorage.getItem("crm_dismissed_admin_esign_notifs");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("crm_dismissed_admin_esign_notifs", JSON.stringify(dismissedNotifIds));
    } catch (e) {
      console.error("Error saving dismissed esign notifications:", e);
    }
  }, [dismissedNotifIds]);

  // Notifications generator dynamically calculated from real documents (matching Tasks.jsx pattern)
  const notifications = useMemo(() => {
    ...
    (Array.isArray(documents) ? documents : []).forEach((doc) => {
      ...
      if (!doc.isSigned && !dismissedNotifIds.includes(notifId)) {
        list.push({
          ...
          message: `Document "${doc.name}" is awaiting signature (Created by ${doc.author || "Admin"}).`,
    ...
    // 2. Recently Signed & Certified Notifications
    (Array.isArray(documents) ? documents : []).forEach((doc) => {
      const docId = doc._id || doc.id;
  ...
  const handleNotifClick = (e, notif) => {
    ...
    if (notif.isRecycled) {
      ...
      navigate(`/e-signatures/editor/${notif.document._id || notif.document.id}`, {
